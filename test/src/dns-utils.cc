...
TEST(DNS_Utils, read_octet) {
    ...
    ASSERT_EQ(covent::dns::utils::read_uint<uint8_t>(ss), 0x47);
}

TEST(DNS_Utils, read_octet_zero) {
    std::string data("\x00", 1);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint8_t>(ss), 0x00);
}

TEST(DNS_Utils, read_short) {
    std::string data("\xFF\x01");
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint16_t>(ss), 0xFF01);
}

TEST(DNS_Utils, read_long) {
    std::string data("\xFF\x01\x02\x03");
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint32_t>(ss), 0xFF010203);
}
...
TEST(DNS_Utils, read_seq) {
    ...
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint16_t>(ss), 0xFF01);
    ASSERT_EQ(covent::dns::utils::read_uint<uint16_t>(ss), 0x0203);
}

TEST(DNS_Utils, read_pstr) {
    std::string data("\x0CHello, World!");
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_pf_string<uint8_t>(ss), "Hello, World");
}

TEST(DNS_Utils, read_short_zero) {
    std::string data("\x00\x00", 2);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint16_t>(ss), 0);
}

TEST(DNS_Utils, read_short_one) {
    std::string data("\x00\x01", 2);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_uint<uint16_t>(ss), 1);
}

TEST(DNS_Utils, read_pstr16) {
    std::string data("\x00\x0CHello, World!", 15);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_pf_string<uint16_t>(ss), "Hello, World");
}

TEST(DNS_Utils, read_octet_empty) {
    std::string data;
    std::istringstream ss(data);
    ASSERT_THROW(covent::dns::utils::read_uint<uint8_t>(ss), std::runtime_error);
}

TEST(DNS_Utils, read_shorter) {
    std::string data("\x00");
    std::istringstream ss(data);
    ASSERT_THROW(covent::dns::utils::read_uint<uint16_t>(ss), std::runtime_error);
}

TEST(DNS_Utils, read_pstr_empty) {
    std::string data("\x00", 1);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_pf_string<uint8_t>(ss), "");
}

TEST(DNS_Utils, read_pstr_short) {
    std::string data("\xFFToo short!");
    std::istringstream ss(data);
    ASSERT_THROW(covent::dns::utils::read_pf_string<uint8_t>(ss), std::runtime_error);
}

TEST(DNS_Utils, read_hostname) {
    std::string data("\x05hello\x5world\x00", 13);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_hostname(ss), "hello.world.");
}

TEST(DNS_Utils, read_hostname_root) {
    std::string data("\x00", 1);
    std::istringstream ss(data);
    ASSERT_EQ(covent::dns::utils::read_hostname(ss), ".");
}

TEST(DNS_Utils, parse_svcb_2) {
    std::string data("\x00\x00\003foo\007example\003com\x00", 19);
    auto rr = covent::dns::rr::SVCB::parse(data);
...
TEST(DNS_Utils, parse_tlsa_1) {
    ...
    auto rr = covent::dns::rr::TLSA::parse(data);
